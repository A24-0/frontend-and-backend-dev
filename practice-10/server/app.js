...
// Кэш access-токенов (jti -> exp) для отзыва при logout
const tokenCache = new Map();
// Сессии: refreshJti -> { userId, createdAt } — для проверки refresh и отслеживания сессий
...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'API Auth + Refresh + Sessions (Практика 10)',
      version: '1.0.0',
      description: 'Refresh-токены, обновление access, отслеживание сессий',
    },
...
// --- Login: access + refresh, cookie, кэш, сессия (Практика 10) ---
app.post('/api/auth/login', async (req, res) => {
  ...
  const accessToken = jwt.sign(
    { sub: user.id, email: user.email, sessionId: refreshJti },
    JWT_SECRET,
...
// --- Refresh: обновление access (и при необходимости refresh) по refresh-токену (Практика 10) ---
app.post('/api/auth/refresh', (req, res) => {
  const refreshToken =
    req.body?.refreshToken ||
    (req.cookies && req.cookies[COOKIE_REFRESH]);
  if (!refreshToken) {
...
// --- Logout: отзыв access и сессии (Практика 10) ---
...
// --- Сессии текущего пользователя (Практика 10) ---
...
app.listen(port, () => {
  ...
  console.log('Refresh-токены: 7d. Сессии отслеживаются. POST /api/auth/refresh, GET /api/auth/sessions');
});
